...
import { graphic_data } from '../../APIs/PortfolioCatApi';
import Banner from '../../components/banner/Banner';
import LinkIcon from '../../assets/icons/linkicon.svg';
import PagArrow from '../../assets/svg/pag-arrow.svg';
import './Portfolio.scss';

const InfograhicDesign = () => {
    ...
    useEffect(() => {
        const getComments = async () => {
            const res = await fetch(
                `http://localhost:3005/infographic_design?_page=1&_limit=${limit}`
            );
    ...
    const fetchComments = async (currentPage) => {
        const res = await fetch(
            `http://localhost:3005/infographic_design?_page=${currentPage}&_limit=${limit}`
        );
        const data = await res.json();
        return data;
    };
...
export default InfograhicDesign;