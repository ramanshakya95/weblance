...
import { content_data } from '../../APIs/PortfolioCatApi';
...
const ProductDescriptions = () => {
    ...
    useEffect(() => {
        const getComments = async () => {
            const res = await fetch(
                `http://localhost:3005/product_description?_page=1&_limit=${limit}`
            );
    ...
    const fetchComments = async (currentPage) => {
        const res = await fetch(
            `http://localhost:3005/product_description?_page=${currentPage}&_limit=${limit}`
        );
        const data = await res.json();
        return data;
    };
