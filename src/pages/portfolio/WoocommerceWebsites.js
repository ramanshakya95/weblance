...
const WoocommerceWebsites = () => {
    ...
    useEffect(() => {
        const getComments = async () => {
            const res = await fetch(
                `https://www.weblance.co.in/dev/api/woocommerce?_page=1&_limit=${limit}`
            );
    ...
    const fetchComments = async (currentPage) => {
        const res = await fetch(
            `https://www.weblance.co.in/dev/api/woocommerce?_page=${currentPage}&_limit=${limit}`
        );
...
export default WoocommerceWebsites;